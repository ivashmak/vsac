...
void Drawer::showMatches (const cv::Mat &img1, const cv::Mat &img2, cv::Mat &img12, const std::vector<bool> &mask,
    ...
    float img_height = 0, img_width = 0;
    if (hor) img_width = (float)(img1.cols+offset);
    else img_height = (float)(img1.rows+offset);
    Drawer::concatenateImage(hor, img1, img2, img12, offset, back);

    if (show_all_points) {
        int outlier_size = ceil((double)inlier_sz / 2);
    ...
    for (int i = 0; i < points1.rows; i++) {
        if (!mask[i]) continue;
        if (random_colors) color = cv::Scalar(rand_gen.uniform(0, 256), rand_gen.uniform(0, 256), rand_gen.uniform(0, 256));
...
void Drawer::drawEpipolarLines (const cv::Mat &F, const cv::Mat &pts1_, const cv::Mat &pts2_, const cv::Mat &img1_, const cv::Mat &img2_,
    ...
    for (int pt = 0; pt < pts_size; pt++) {
        if (!inliers[pt]) continue;
        if (random_colors)
            col=cv::Scalar (rng.uniform(0, 256), rng.uniform(0, 256), rng.uniform(0, 256));
