...
private:
    ...
    Ptr<FundamentalNonMinimalSolver> f_solver;
public:
    /*
    ...
    explicit EssentialNonMinimalSolverViaFImpl(const Mat &points_, const Mat &K1_, const Mat &K2_) :
            K1(K1_), K2_t(K2_.t()), f_solver (FundamentalNonMinimalSolver::create(points_)) {
//        std::cout << points_ << "\n";
    }
    ...
    int estimate (const std::vector<int> &sample, int sample_size, std::vector<Mat>&models, const std::vector<double> &weights) const override {
        int sols = f_solver->estimate(sample, sample_size, models, weights);
        if (sols == 0) return 0;
        models[0] = cv::Mat(cv::Matx33d(K2_t) * cv::Matx33d(models[0]) * cv::Matx33d(K1));
        if (enforce_rank) {
            std::cout << "force rank\n";
...
class EssentialMinimalSolverStewenius5ptsSVDImpl : public EssentialMinimalSolverStewenius5ptsSVD {
private:
    // Points must be calibrated K^-1 x
    const Mat * points_mat;
#if defined(HAVE_EIGEN) || defined(HAVE_LAPACK)
...
#endif
public:
    explicit EssentialMinimalSolverStewenius5ptsSVDImpl (const Mat &points_) :
            points_mat(&points_)
#if defined(HAVE_EIGEN) || defined(HAVE_LAPACK)
            , pts((float*)points_.data)
#endif
    ...
    int estimate (const std::vector<int> &sample, std::vector<Mat> &models) const override {
        ...
        const int num_cols = 9, num_e_mat = 4;
        double ee[36]; // 9*4
        cv::Matx<double, 5, 9> coeffs (&coefficients[0]);
        cv::Mat D, U, Vt;
        SVDecomp(coeffs, D, U, Vt, cv::SVD::FULL_UV + cv::SVD::MODIFY_A);
        const auto * const vt = (double *) Vt.data;
        for (int i = 0; i < num_e_mat; i++)
            for (int j = 0; j < num_cols; j++)
                // ee[i * num_cols + j] = vt[(5+i)*num_cols+j];
...
Ptr<EssentialMinimalSolverStewenius5ptsSVD> EssentialMinimalSolverStewenius5ptsSVD::create
        (const Mat &points_) {
    return makePtr<EssentialMinimalSolverStewenius5ptsSVDImpl>(points_);
}
...
public:
    ...
    int estimate (const Mat &E, const std::vector<int> &sample, int sample_size, std::vector<Mat>
        ...
        cv::Matx33d Cov1 = cv::Matx33d::zeros(), Cov2 = cv::Matx33d::zeros();
        ...
        cv::Matx33d Vt1, Vt2, U;
        cv::Vec3d D;
        cv::SVDecomp(Cov1, D, U, Vt1, cv::SVD::FULL_UV + cv::SVD::MODIFY_A);
        cv::SVDecomp(Cov2, D, U, Vt2, cv::SVD::FULL_UV + cv::SVD::MODIFY_A);
        models = std::vector<Mat> {
                cv::Mat(cv::vsac::Math::getSkewSymmetric(cv::Vec3d(Vt1(2, 0), Vt1(2, 1), Vt1(2, 2))) * R1),
                cv::Mat(cv::vsac::Math::getSkewSymmetric(cv::Vec3d(Vt2(2, 0), Vt2(2, 1), Vt2(2, 2))) * R2) };
        return 2;
    }
    int getMinimumRequiredSampleSize() const override { return 8; }
    int getMaxNumberOfSolutions () const override { return 1; }
