...
public:
	...
	void getTandEpipoles (const Mat &tensor, std::vector<cv::Matx33d> &T, cv::Vec3d &e_prime, cv::Vec3d &e_prime_prime) {
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
				for (int k = 0; k < 3; k++)
					T[i](j,k) = ten[cnt++];
		std::vector<cv::Vec3d> left_null(3), right_null(3);		
		for (int i = 0; i < 3; i++) {
			right_null[i] = getNullVector(cv::Mat(T[i]));
			left_null[i] = getNullVector(cv::Mat(T[i].t()));
		}
		/*
	...
	void getFundamentalMatricesFromTensor (const cv::Mat &tensor, cv::Mat &F21, cv::Mat &F31) {
		...
		F21 = cv::Mat_<double>(3,3);
		F31 = cv::Mat_<double>(3,3);
		for (int i = 0; i < 3; i++) {
	...
	void getProjectionsFromTensor (const cv::Mat &tensor, cv::Matx34d &P2, cv::Matx34d &P3) {
		...
		// e' = a4, e'' = b4
		cv::Mat P2_ = cv::Mat_<double>(3,4,P2.val), P3_ = cv::Mat_<double>(3,4,P3.val);
		cv::Mat(a4).copyTo(P2_.col(3));
		cv::Mat(b4).copyTo(P3_.col(3));
		cv::Matx33d bI = (b4 * b4.t() - cv::Matx33d::eye());
		for (int i = 0; i < 3; i++) {
			cv::Mat(T[i] * b4).copyTo(P2_.col(i));
			cv::Mat(bI * T[i].t() * a4).copyTo(P3_.col(i));
		}
	}

	// https://www.robots.ox.ac.uk/~vgg/publications/1997/Torr97a/torr97a.pdf
	cv::Matx14d cross4 (const cv::Vec4d &a, const cv::Vec4d &b, const cv::Vec4d &c) const {
	    const double l1 = cv::determinant(cv::Matx33d(a[1], a[2], a[3],
	    											  b[1], b[2], b[3],
	    											  c[1], c[2], c[3])),
	      			 l2 = -cv::determinant(cv::Matx33d(a[0], a[2], a[3],
	    											   b[0], b[2], b[3],
	    											   c[0], c[2], c[3])),
				     l3 = cv::determinant(cv::Matx33d(a[0], a[1], a[3],
	    											  b[0], b[1], b[3],
	    											  c[0], c[1], c[3])),
	...
	int estimate(const std::vector<int> &sample, std::vector<cv::Mat> &tensor) const override {
		cv::Matx<double, 3, 5> A;
		auto getLambda = [&] (const int img, cv::Matx33d &B, cv::Vec3d &X5, cv::Vec3d &X6) {
		...
		auto getCoeffs = [] (double ai, double bi, double aj, double bj, double ak, double bk) {
			return cv::Vec4d(ai*aj*ak, ai*aj*bk+ai*bj*ak+ai*aj*ak, ai*aj*bk+bi*bj*ak+ai*bj*bk, bi*bj*bk);
		};

		std::vector<cv::Matx33d> Bs(3);
		std::vector<cv::Vec3d> X5s(3), X6s(3);
	    ...
	    cv::vsac::Math::eliminateUpperTriangular(A_vec, A.rows, A.cols);
	    double f1[5], f2[5];
		...
		cv::Vec4d coeffs = getCoeffs(a1,b1,a2,b2,a5,b5)-getCoeffs(a2,b2,a3,b3,a5,b5)
