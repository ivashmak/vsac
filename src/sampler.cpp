...
class QuasiUniformSamplerImpl : public QuasiUniformSampler {
private:
    std::vector<int> points_random_pool;
...
public:
    ...
    void updateSampler (const std::vector<bool> &inliers) override {
        do_quasi = true;
        int idx1 = 0, idx2 = points_size-1;//, r_idx = rng.uniform(0, points_size);
        for (int i = 0; i < points_size; i++) {
            const int r_pt = points_random_pool[i];
            if (inliers[r_pt])
                points_random_pool[idx1++] = r_pt;
            else points_random_pool[idx2--] = r_pt;
        }
        pool_idx = 0;
        num_inliers = idx1;
        ...
        for (int i = points_size; i >= num_inliers+1; i--)
            std::swap(points_random_pool[rng.uniform(num_inliers, i)], points_random_pool[i-1]);
    }
    void setNewPointsSize (int points_size_) override {
        setPointsSize(points_size_);
    }
...
Ptr<QuasiUniformSampler> QuasiUniformSampler::create(int state, int sample_size_, int points_size_) {
    return makePtr<QuasiUniformSamplerImpl>(state, sample_size_, points_size_);
}
