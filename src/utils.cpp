...
private:
    ...
    constexpr static int BIS_MAX_ITERS = 500, NEWTON_MAX_ITERS = 200, REGULA_FALSI_MAX_ITERS = 500, MAX_POWER = 10, MAX_LEVEL = 200;
    constexpr static double TOLERANCE = 1e-10, DIFF_TOLERANCE = 1e-7;

    static bool findRootNewton (const Poly &poly, double x0, double &root) {
        const Poly derivative = poly.derivative();
        int iter = 0;
        for (; iter < NEWTON_MAX_ITERS; iter++) {
            const double diff = poly.eval(x0) / derivative.eval(x0);
            if (fabs(diff) < DBL_EPSILON)
                break;
            root = x0 - diff;
            if (fabs(poly.eval(root)) < DBL_EPSILON) {
                std::cout << "Newton root " << root << " is found at iter " << iter << " eval " << poly.eval(root) << "\n";
                return true; // root is found
            }
            x0 = root;
        }
        std::cout << "Newton diverged, iters made " << iter << ", val "<< root << " eval " << poly.eval(root) << "\n";
        if (poly.eval(root) < 1e-3) {
            std::cout << "Newton accept diverged root\n";
            return true;
        }
        return false;
    }
    ...
    static bool findRootBisection (const Poly &poly, double min, double max, double &root) {
        double f_min = poly.eval(min), f_max = poly.eval(max);
        if (f_min * f_max > 0 || min > max) {// conditions are not fulfilled
            // std::cout << "conditions are not fulfilled, min " << min << " max " << max << " f(min) " << poly.eval(min) << " f(max) " << poly.eval(max) << "\n";
            return false;
        }
        int iter = 0;
        while (iter++ < BIS_MAX_ITERS) {
            root = (min + max) * 0.5;
            const auto f_mid = poly.eval(root);
            if (fabs(f_mid) < TOLERANCE || fabs((min-max)*.5) < DIFF_TOLERANCE) {
                return true; // root is found
            }
            if (f_mid * f_min > 0) {
                min = root;
...
public:
    int getRealRoots (const std::vector<double> &coeffs, std::vector<double> &real_roots) override {
        ...
        ///////////////////////////////
        if (real_roots.size() > input.degree())
...
int Utils::removeClosePoints (const Mat &points, Mat &new_points, float thr) {
    const auto graph_builder = GridNeighborhoodGraph2Images::create(points, points.rows, thr, thr, thr, thr);
    // precise method:
// append:
//    const auto graph_builder1 = RadiusSearchNeighborhoodGraph::create(points.colRange(0,2).clone(), points.rows, thr, 8, 1);
//    const auto graph_builder2 = RadiusSearchNeighborhoodGraph::create(points.colRange(2,4).clone(), points.rows, thr, 8, 1);

    const auto &graph = graph_builder->getGraph();
    std::vector<bool> unique_pts(points.rows, true);
    for (int i = 0; i < points.rows; i++)
        for (int n : graph[i])
            unique_pts[n] = false;
    int num_unique_pts = 0;
    for (auto is_unique : unique_pts)
        if (is_unique)
            num_unique_pts++;

    new_points = Mat_<float>(num_unique_pts, 4);
...
bool getCorrectedPointsHomography(const cv::Mat &points1, const cv::Mat &points2, cv::Mat &corr_points1, cv::Mat &corr_points2, const cv::Mat &H, const std::vector<bool> &mask) {
    ...
    for (int i = 0; i < points1.rows; i++) {
        ...
        double s1_sqr = 1/cv::determinant(A / A(2,2));
        s1_sqr *= s1_sqr;
        // H^-1 (x2 y2 1)
