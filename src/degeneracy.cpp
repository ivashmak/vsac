...
private:
    ...
    const float * const points_sample, * const points_val;
    const Mat * points_mat_sample, * points_mat_val;
    const Ptr<ReprojectionErrorForward> h_reproj_error_sample, h_reproj_error_val;
    ...
    const int points_size_sample, points_size_val, sample_size, max_iters_plane_and_parallax, TENT_MIN_NON_PLANAR_SUPP = 10, MAX_H_SUBSET = 50, MAX_ITERS_H = 6;
...
public:
    FundamentalDegeneracyImpl (int state, const Ptr<Quality> &quality_, const Mat &points_sample_, const Mat &points_val_,
            ...
            rng (state), quality(quality_), f_error(quality_->getErrorFnc()), points_sample((float *) points_sample_.data),
            points_val((float *)points_val_.data), points_mat_sample(&points_sample_), points_mat_val(&points_val_),
            h_reproj_error_sample(ReprojectionErrorForward::create(points_sample_)),
            h_reproj_error_val(ReprojectionErrorForward::create(points_val_)),
            ep_deg (points_sample_, sample_size_), homography_threshold (homography_threshold_),
            points_size_sample (points_sample_.rows), points_size_val (points_val_.rows), sample_size (sample_size_),
        ...
        h_inliers = std::vector<int>(points_size_val);
        h_outliers = std::vector<int>(points_size_val);
        new_h_outliers = std::vector<int>(points_size_val);
        h_non_min_solver = HomographyNonMinimalSolver::create(points_val_);
        plane_parallax_H_sample = std::vector<int>(points_size_val);
        num_h_outliers = points_size_val;
        f_threshold_sqr = f_inlier_thr_sqr;
        H_best_score = Score();
        h_repr_quality = MsacQuality::create(points_val_.rows, homography_threshold_, h_reproj_error_val);
        ...
        random_gen_H = UniformRandomGenerator::create(rng.uniform(0, INT_MAX), points_size_val, MAX_H_SUBSET);
    ...
    bool estimateHfrom3Points (const Mat &F_best, const std::vector<int> &sample) {
        ...
        for (const auto &h_i : h_sample) { // only 5 samples
            Matx33d H;
            if (!getH(A, e_prime, 4*sample[h_i[0]], 4*sample[h_i[1]], 4*sample[h_i[2]], H, true))
                continue;
            h_reproj_error_sample->setModelParameters(Mat(H));
            int inliers_in_plane = 0;
            for (int s = 0; s < sample_size; s++)
                if (h_reproj_error_sample->getError(sample[s]) < homography_threshold)
    ...
    bool planeAndParallaxRANSAC (const Matx33d &H, std::vector<int> &non_planar_pts, int num_non_planar_pts,
        ...
        for (; iters < max_iters; iters++) {
            ...
            const Matx33d F = Math::getSkewSymmetric(
                    (Vec3d(points_val[4*h_outlier1+2], points_val[4*h_outlier1+3], 1).cross   // p1'
               (H * Vec3d(points_val[4*h_outlier1  ], points_val[4*h_outlier1+1], 1))).cross // Hp1
                     (Vec3d(points_val[4*h_outlier2+2], points_val[4*h_outlier2+3], 1).cross   // p2'
               (H * Vec3d(points_val[4*h_outlier2  ], points_val[4*h_outlier2+1], 1)))       // Hp2
            ) * H;
...
#ifdef DEBUG_DEGENSAC
            std::cout << "Plane-and-parallax failed, support " << non_planar_support << " estimated " << estimated_min_non_planar_support << "\n";
#endif
            return false;
        }
    ...
    bool calibDegensac (const Matx33d &H, Mat &F_new, Score &F_new_score, const Mat &F_degen_best, const Score &F_degen_score) {
        if (! is_principal_pt_set) {
            ...
            for (int i = 0; i < points_size_val; i++) {
                const int idx = 4*i;
                if (px1 < points_val[idx  ]) px1 = points_val[idx  ];
                if (py1 < points_val[idx+1]) py1 = points_val[idx+1];
                if (px2 < points_val[idx+2]) px2 = points_val[idx+2];
                if (py2 < points_val[idx+3]) py2 = points_val[idx+3];
            }
            for (int i = 0; i < points_size_sample; i++) {
                const int idx = 4*i;
                if (px1 < points_sample[idx  ]) px1 = points_sample[idx  ];
                if (py1 < points_sample[idx+1]) py1 = points_sample[idx+1];
                if (px2 < points_sample[idx+2]) px2 = points_sample[idx+2];
                if (py2 < points_sample[idx+3]) py2 = points_sample[idx+3];
            }
...
//                      " inliers " << quality->getScore(F1).inlier_number << " "<< quality->getScore(F2).inlier_number << "\n";
...
#ifdef DEBUG_DEGENSAC
            std::cout << "Calibrated DEGENSAC failed, support " << non_planar_support_out << " estimated " << estimated_min_non_planar_support << "\n";
#endif
            return false;
        }
    ...
    void getOutliersH (const Mat &degen_F) {
        ...
        num_h_outliers = 0;
        plane_parallax_H_sample_size = 0;
        const auto &h_errors = h_reproj_error_val->getErrors(H_best);
        for (int pt = 0; pt < points_size_val; pt++)
    ...
    bool verifyFundamental (const Mat &F_best, const Score &F_score, const std::vector<bool> &inliers_mask, Mat &F_new, Score &new_score) override {
        if (true_K_given && areSameSingularValues(F_best))
            return false;
#ifdef DEBUG_DEGENSAC
        std::cout << "verify F: best score " << F_score.score << " " << F_score.inlier_number << '\n';
#endif
        const int f_sample_size = 3, max_H_iters = 5; // 3.52 = log(0.01) / log(1 - std::pow(0.9, 3));
        int num_f_inliers = 0;
        std::vector<int> inliers(points_size_val), f_sample(f_sample_size);
        for (int i = 0; i < points_size_val; i++) if (inliers_mask[i]) inliers[num_f_inliers++] = i;
        ...
        for (int iter = 0; iter < max_H_iters; iter++) {
            ...
            Matx33d H;
            if (!getH(A, e_prime, 4*inliers[f_sample[0]], 4*inliers[f_sample[1]], 4*inliers[f_sample[2]], H, false))
                continue;
            const auto h_score = h_repr_quality->getScore(Mat(H));
            if (h_score.isBetter(H_best_score)) {
...
private:
    ...
    bool getH (const Matx33d &A, const Vec3d &e_prime, int smpl1, int smpl2, int smpl3, Matx33d &H, bool is_sample) {
        Vec3d p1, p2, p3, P1, P2, P3;
        if (is_sample) {
            p1 = {points_sample[smpl1  ], points_sample[smpl1+1], 1}; p2 = {points_sample[smpl2  ], points_sample[smpl2+1], 1}; p3 = {points_sample[smpl3  ], points_sample[smpl3+1], 1}; 
            P1 = {points_sample[smpl1+2], points_sample[smpl1+3], 1}; P2 = {points_sample[smpl2+2], points_sample[smpl2+3], 1}; P3 = {points_sample[smpl3+2], points_sample[smpl3+3], 1}; 
        } else {
            p1 = {points_val[smpl1  ], points_val[smpl1+1], 1}; p2 = {points_val[smpl2  ], points_val[smpl2+1], 1}; p3 = {points_val[smpl3  ], points_val[smpl3+1], 1}; 
            P1 = {points_val[smpl1+2], points_val[smpl1+3], 1}; P2 = {points_val[smpl2+2], points_val[smpl2+3], 1}; P3 = {points_val[smpl3+2], points_val[smpl3+3], 1}; 
        }
...
Ptr<FundamentalDegeneracy> FundamentalDegeneracy::create (int state, const Ptr<Quality> &quality_,
        const Mat &points_sample, const Mat &points_val, int sample_size_, int max_iters_plane_and_parallax, double homography_threshold_,
        double f_inlier_thr_sqr, const Mat true_K1, const Mat true_K2) {
    return makePtr<FundamentalDegeneracyImpl>(state, quality_, points_sample, points_val, sample_size_,
