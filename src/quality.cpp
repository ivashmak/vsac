...
private:
    ...
    const int points_size;
    int highest_inlier_number, current_sprt_idx;
    ...
    inline bool isModelGood (const Mat &model) override {
            ...
            if (score_type == ::vsac::ScoreMethod::SCORE_METHOD_MSAC) {
                ...
                for (tested_point = 0; tested_point < points_size; tested_point++) {
                    ...
                    if (error < inlier_threshold) {
                        ...
                        if (lambda > current_A)
                            break;
                        if (tested_inliers - tested_point < preemptive_thr)
                            break;
                    }
                    errors[pt] = error;
                }
        ...
        if (! last_model_is_good) {
            ...
            if (m_score.inlier_number > highest_inlier_number) {
                std::cout << "MODEL WITH HIGHEST NUM OF INLIERS REJECTED (" << m_score.score << ", " << m_score.inlier_number << ") BEST " << highest_inlier_number << " FN ratio " << 1 / sprt_histories[current_sprt_idx].A << " eps " << current_epsilon << " rej eps " << m_score.inlier_number / (double) points_size << '\n';
            }
        }
#endif
...
Ptr<AdaptiveSPRT> AdaptiveSPRT::create (int state, const Ptr<Quality> &quality, int points_size_,
            double inlier_threshold_, double prob_pt_of_good_model, double prob_pt_of_bad_model,
            double time_sample, double avg_num_models, ::vsac::ScoreMethod score_type_, double k_mlesac, bool is_adaptive) {
