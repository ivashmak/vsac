...
#include "../lib/relative_pose/bundle.h"
...
public:
    ...
    int estimate (const Mat &model, const std::vector<int> &sample, int sample_size, std::vector<Mat>
        ...
        for (int i = 0; i < 3; i++) { // could be 1 point
            ...
            if (Utils::satisfyCheirality(R1,  t, p1, p2)) positive_depth[0]++;
            if (Utils::satisfyCheirality(R1, -t, p1, p2)) positive_depth[1]++;
            if (Utils::satisfyCheirality(R2,  t, p1, p2)) positive_depth[2]++;
            if (Utils::satisfyCheirality(R2, -t, p1, p2)) positive_depth[3]++;
        }
        ...
        pose_lib::refine_relpose(calib_points, sample, sample_size, &pose, opt, &weights[0]);
