...
public:
    ...
    virtual bool recoverIfDegenerate (const std::vector<int> &/*sample*/,const Mat &/*best_model*/, const Score &/*score*/,
                  Mat &/*non_degenerate_model*/, Score &/*non_degenerate_model_score*/, int &/*non_planar_support*/) {
        return false;
    }

    virtual const std::vector<int> &getGoodSample() {
        return good_sample;
    }
...
public:
    ...
    static Ptr<AdaptiveSPRT> create (int state, const Ptr<Error> &err_, const Ptr<Quality> &quality, int points_size_,
         double inlier_threshold_, double prob_pt_of_good_model, double prob_pt_of_bad_model,
         double time_sample, double avg_num_models, ::vsac::ScoreMethod score_type_, int max_iters_to_adapt,
...
public:
    virtual int getMinTerminationLength () const = 0;
...
public:
    static Ptr<WeightedPolisher> create (const Ptr<Degeneracy> &degeneracy, const Ptr<Quality> &quality_, const Ptr<NonMinimalSolver> &solver_, const Ptr<Error> &error_fnc_,
...
class IterativePolisher : public FinalModelPolisher {
public:
    static Ptr<IterativePolisher> create (const Ptr<Quality> &quality_, const Ptr<Degeneracy> &degen, const Ptr<Estimator> &estimator_,
