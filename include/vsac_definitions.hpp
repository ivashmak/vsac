...
public:
    ...
    static Ptr<FundamentalDegeneracy> create (int state, const Ptr<Quality> &quality_,
        const Mat &points_sample, const Mat &points_val_, int sample_size_, int max_iters_plane_and_parallax,
...
protected:
    ...
    Ptr<Error> _error_sample, _error_val;
    Ptr<Quality> _quality, quality_sample;
    Ptr<Sampler> _sampler;
    Ptr<Termination> _termination, _lo_termination;
    ...
    int points_sample_size, points_val_size, _state;
    bool parallel;

    Matx33d T1, T2;
    Mat points_sample, points_val, K1, K2, calib_points_val, calib_points_sample, image_points_sample, image_points_val;
